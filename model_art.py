...
class ARTPolicy(PreTrainedPolicy):
    ...
    def _prepare_batch(self, batch: dict[str, Tensor]) -> dict[str, Tensor]:
        if self.config.image_features:
            ...
            images = [self._select_image_frame(batch[key]) for key in self.config.image_features]
            batch[OBS_IMAGES] = torch.stack(images, dim=1)
        return batch

    def _select_image_frame(self, img: Tensor) -> Tensor:
        if img.dim() == 5:
            # print(f"Received image tensor with shape {tuple(img.shape)}. Selecting frame at index {min(self.config.history_length, img.shape[1] - 1)} for inference.")
            time_index = min(self.config.history_length, img.shape[1] - 1)
            return img[:, time_index]
        if img.dim() == 4:
            return img
        raise ValueError(
            f"Expected image tensor to be 4D or 5D, got shape {tuple(img.shape)}."
        )

    @torch.no_grad()
    def predict_action_chunk(self, batch: dict[str, Tensor]) -> Tensor:
        """Predict the next action step as a single-step chunk."""
        self.eval()
        actions = self._predict_next_action(batch)
        return actions.unsqueeze(1)

    def _predict_next_action(self, batch: dict[str, Tensor]) -> Tensor:
        raw_state = None
        if self.config.tokenize_actions and self.config.tokenize_delta_actions:
            raw_state = batch.get("observation.state_raw", batch.get(OBS_STATE))

        batch = self._prepare_batch(batch)
        if self.test_step_counter % self.config.n_action_steps == 0:
            self.model.update_vision_prefix(batch)

        out_dict = self.model.generate_next_action(batch)
        actions_raw = out_dict.get("action_out").squeeze(1)
        ...
        if self.config.tokenize_actions:
            ...
            if self.config.tokenize_delta_actions and raw_state is not None:
                if raw_state.dim() == 3:
                    raw_state = raw_state[:, -1, :]
                actions = actions + raw_state
        else:
            actions = actions_raw
    ...
    def select_action(self, batch: dict[str, Tensor]) -> Tensor:
        ...
        """
        self.eval()
        return self._predict_next_action(batch)

    def forward(self, batch: dict[str, Tensor]) -> tuple[Tensor, dict[str, float]]:
        """Run the batch through the model and compute the loss for training or validation."""
        cache_current_state = None
        if self.config.tokenize_actions and self.config.tokenize_delta_actions:
            cache_current_state = batch.get("observation.state_raw", batch.get(OBS_STATE))
        ...
        if self.config.tokenize_actions:
            ...
            with torch.no_grad():
                if self.config.tokenize_delta_actions and cache_current_state is not None:
                    gt_actions = gt_actions - cache_current_state[:, -self.config.chunk_size :, :]
            ...
            mask = mask.unsqueeze(-1).expand(-1, -1, Dim)

            ar_loss = (
                F.cross_entropy(actions_hat_permuted, action_gt_tokens, reduction="none") * mask
            ).mean()

            fast_loss = (
                F.cross_entropy(actions_fast_permuted, action_gt_tokens, reduction="none") * mask
            ).mean()

            loss = ar_loss + fast_loss
        else:
            ar_loss = (
            ...
            fast_loss = (
                F.l1_loss(batch[ACTION], actions_fast, reduction="none")
                * ~batch["action_is_pad"].unsqueeze(-1)
            ).mean()

            loss = ar_loss + fast_loss

        loss_dict = {"ar_loss": ar_loss.item(), "fast_loss": fast_loss.item(), "loss": loss.item()}
